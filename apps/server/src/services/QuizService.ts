...
export class QuizService {
  ...
  async startQuiz(
    ...
    const rawQuestions = await this.questionRepo.findByTopicAndDifficulty(topic, difficulty, mode, count, subtopic);
  ...
  async getTestCases(quizId: string, userId: string, questionId: string) {
    ...
    const question = await this.questionRepo.findById(questionId);
    if (!question) throw { statusCode: 404, message: 'Question not found' };
    if (question.type !== 'code') throw { statusCode: 400, message: 'Not a code question' };
  ...
  async completeQuiz(quizId: string, userId: string) {
    ...
    if (user) {
      ...
      // Update streak
      const now = new Date();
      const lastActive = user.lastActiveDate || now;
  ...
  async getProfileStats(userId: string) {
    ...
    const result = stats[0] || { totalQuizzes: 0, totalCorrect: 0, totalAnswered: 0, totalTime: 0 };
    const accuracy = result.totalAnswered > 0
    ...
    return {
      ...
      accuracy,
      totalTime: totalHours > 0 ? `${totalHours} soat` : `${totalMinutes} daqiqa`
    };
  }
  ...
  async getGrowthData(userId: string) {
    ...
    return monthlyData.map((m: any) => ({
      ...
      xp: m.xp,
      accuracy: m.totalAnswered > 0 ? Math.round((m.totalCorrect / m.totalAnswered) * 100) : 0
    }));
  }
  ...
  async getSkillStats(userId: string) {
    ...
    return skills.map((s: any) => ({
      ...
      value: s.totalAnswered > 0 ? Math.round((s.totalCorrect / s.totalAnswered) * 100) : 0,
