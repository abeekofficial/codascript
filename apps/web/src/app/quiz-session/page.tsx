...
export default function TestJarayoni() {
  ...
  useEffect(() => {
    if (question) {
      api
        .checkSaved("question", question.id)
        .then(setIsSaved)
        .catch(console.error);
    }
  }, [question?.id]);
  ...
  return (
    <div className="flex min-h-screen w-full flex-col text-ink">
          ...
          <div className="min-w-0">
            <p className="flex items-center gap-2 text-sm font-semibold">
              ...
              <span className="truncate">
                {tech.label} <span className="text-ink-muted">•</span>{" "}
                {difficultyLabel}
              </span>
            </p>
      ...
      <div
        className={`mx-auto w-full max-w-7xl flex-1 px-5 py-7 lg:px-8 ${question.type === "code" ? "flex flex-col" : "grid gap-6 lg:grid-cols-[minmax(0,1fr)_300px]"}`}
      >
          ...
          >
            ...
            {question.type !== "code" && (
              <div className="flex items-center justify-between mt-4">
                <h1 className="text-lg font-semibold leading-snug lg:text-xl pr-4">
                  {question.question}
                </h1>
                <button
                  onClick={async () => {
                ...
                >
                  <BookmarkIcon
                    className={`h-5 w-5 ${isSaved ? "fill-neon text-neon" : "text-ink-dim"}`}
                  />
                </button>
              </div>
            )}
              ...
              >
                {question.options.map((option, i) => {
                  ...
                  const isActuallyCorrect =
                    answered &&
                    !answered.isCorrect &&
                    answered.correctAnswerText === option;
                  ...
                  return (
                    <button
                      ...
                      className={[
                        ...
                        isSelectedCorrect || isActuallyCorrect
                          ? "border-neon bg-neon/10"
                          : isSelectedWrong
                            ? "border-danger bg-danger/10"
                            : !answered &&
                                answers[question.id]?.selectedOptionIndex === i
                              ? "border-neon bg-neon/10"
                    ...
                    >
                      ...
                      <span
                        className={`font-mono text-sm ${active || isActuallyCorrect ? "text-ink" : "text-ink-dim"}`}
                      >
                        {option}
                      </span>
                    </button>
            ...
            {answers[question.id] && answers[question.id].explanation && (
              ...
              >
                ...
                <p className="text-sm text-ink-dim">
                  {answers[question.id].explanation}
                </p>
              </motion.div>
            )}
          </motion.section>
                ...
                {questions.map((q, i) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        useQuizStore.setState({ currentQuestionIndex: i })
                      }
