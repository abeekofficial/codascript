...
function verdict(pct: number) {
  if (pct >= 85) return { title: 'A’lo natija!', text: 'Siz bu mavzuni ishonchli egallagansiz.', color: '#10B981' };
  if (pct >= 65) return { title: 'Yaxshi natija', text: 'Bir necha mavzuni mustahkamlash kifoya.', color: '#FBBF24' };
  return { title: 'Mashq kerak', text: 'Xatolar ustida ishlab, testni qayta yeching.', color: '#F87171' };
}
...
export default function Natijalar() {
  ...
  const xp = correct * 50; // default calc
  ...
  const stats = [
    { id: 'correct', label: 'To‘g‘ri', value: correct, icon: CheckCircle2Icon, color: '#10B981' },
    { id: 'wrong', label: 'Noto‘g‘ri', value: wrong, icon: XCircleIcon, color: '#F87171' },
    { id: 'skipped', label: 'O‘tkazib yuborilgan', value: skipped, icon: MinusCircleIcon, color: '#8B949E' },
    { id: 'xp', label: 'Yig‘ilgan XP', value: xp, icon: ZapIcon, color: '#FBBF24' }
  ];
    ...
    <div className="mx-auto max-w-5xl">
      <section className="rounded-2xl border border-line bg-surface px-6 py-10 text-center">
        ...
        <div className="relative mx-auto mt-7 h-52 w-52">
          <svg viewBox="0 0 200 200" className="h-full w-full -rotate-90">
            <circle cx="100" cy="100" r={radius} fill="none" stroke="#1F2937" strokeWidth="14" />
            <motion.circle
              cx="100"
              cy="100"
              r={radius}
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-5xl font-extrabold tabular-nums tracking-tight">{pct}%</span>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {stats.map(({ id, label, value, icon: Icon, color }) => (
          <div key={id} className="flex flex-col rounded-2xl border border-line bg-surface p-5">
            <span
            ...
            <span className="mt-4 text-3xl font-bold tabular-nums" style={{ color }}>
              {value}
            </span>
      ...
      <section className="mt-6 rounded-2xl border border-line bg-surface p-6">
        <h2 className="text-base font-semibold">Ko‘rib chiqish uchun savollar (Xatolar)</h2>
        <ul className="mt-4 divide-y divide-line">
          {mistakes.slice(0, 3).map((q, i) => (
            <li key={q.id} className="flex items-start gap-4 py-4 first:pt-0 last:pb-0">
          ...
          {mistakes.length === 0 && (
            <p className="text-sm text-ink-muted">Ajoyib! Hech qanday xato yo'q.</p>
          )}
        </ul>
      </section>
      ...
      {Object.keys(topicBreakdown).length > 1 && (
        <section className="mt-6 rounded-2xl border border-line bg-surface p-6">
          <h2 className="text-base font-semibold mb-4">Mavzular bo'yicha tahlil</h2>
          <div className="grid gap-3 sm:grid-cols-2">
            {Object.entries(topicBreakdown).map(([t, stats]) => {
              ...
              const techEntry = Object.values(TECH_MAP).find(tech => tech.label.toLowerCase() === t.toLowerCase());
              const techObj = techEntry || { label: t, color: '#10B981' };
              const perc = Math.round((stats.correct / stats.total) * 100);
              return (
                <div key={t} className="flex items-center justify-between rounded-xl bg-elevated p-4 border border-line">
                  <div className="flex items-center gap-3">
                    <span className="h-3 w-3 rounded-full" style={{ backgroundColor: techObj.color }} />
                    <span className="font-semibold text-sm">{techObj.label}</span>
                  </div>
                  <div className="text-right">
                    <span className={`text-sm font-bold ${perc >= 70 ? 'text-success' : perc >= 40 ? 'text-warning' : 'text-danger'}`}>
                      {stats.correct} / {stats.total} to'g'ri
                    </span>
                    <p className="text-xs text-ink-muted mt-0.5">{perc}% aniqlik</p>
                  </div>
                </div>
              );
            })}
