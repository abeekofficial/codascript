...
export async function generateMetadata({
  ...
  const topicName =
    params.topic.charAt(0).toUpperCase() + params.topic.slice(1);
  return {
    ...
    keywords: [
      `learn ${params.topic}`,
      `${params.topic} tutorials`,
      `${params.topic} quizzes`,
      `web development`,
    ],
    alternates: {
...
async function getTopicStats(topic: string) {
  ...
  try {
    const res = await fetch(
      `${url}/questions/count?topic=${topic}&difficulty=mixed&mode=topic`,
  ...
  return (
    <div className="container py-24 px-4 max-w-4xl mx-auto min-h-screen">
      <div className="mb-8">
        <Link
          href="/learn"
          className="text-primary font-mono text-sm hover:underline uppercase flex items-center gap-2 mb-6"
        >
        ...
        <p className="text-muted-foreground font-mono text-lg md:text-xl leading-relaxed">
          Welcome to the definitive guide to {topicName}. Whether you are a
          beginner looking to understand the fundamentals or an experienced
          developer testing your advanced knowledge, our interactive quizzes are
          designed to push your skills to the next level.
        </p>
      </div>
        ...
        <div className="glass-card p-6 border-l-4 border-l-primary flex flex-col justify-between">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-mono font-bold text-white uppercase text-xl">
                Test Your Knowledge
              </h3>
            </div>
            <p className="text-muted-foreground font-mono text-sm mb-6">
              Challenge yourself with our curated database of{" "}
              {totalQuestions > 0 ? totalQuestions : "numerous"} questions
              spanning various difficulty levels.
            </p>
          </div>
          <Link
            href={`/quiz/${params.topic.toLowerCase()}`}
            className="inline-block bg-primary text-primary-foreground text-center py-3 font-mono uppercase tracking-widest hover:bg-primary/90 transition-colors font-bold"
          >
          ...
          <div>
            ...
            <ul className="text-muted-foreground font-mono text-sm space-y-3 mb-6">
              <li className="flex items-center gap-2">
                <span className="text-primary">&gt;</span> Fundamentals & Syntax
              </li>
              <li className="flex items-center gap-2">
                <span className="text-primary">&gt;</span> Advanced Concepts
              </li>
              <li className="flex items-center gap-2">
              ...
              <li className="flex items-center gap-2">
                <span className="text-primary">&gt;</span> Common Pitfalls
              </li>
            </ul>
          </div>
        </div>
